...
def run_cmd(cmd: list[str]) -> tuple[int, float, str, str]:
    t0 = time.time()
...
def main(cfg_path: str) -> None:
    ...
    for r in range(int(cfg["rounds"])):
        # Prove (placeholder)
        rc_p, prove_ms, out_p, err_p = run_cmd(
            FEDZK_CLI + ["generate", "--round", str(r)]
        )
        # Verify (placeholder)
        rc_v, verify_ms, out_v, err_v = run_cmd(
            FEDZK_CLI + ["verify", "--round", str(r)]
        )
        ...
        (ART / "transcripts" / f"round_{r:03d}.json").write_text(
